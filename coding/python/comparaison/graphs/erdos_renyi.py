...
def generate_erdos_renyi_graph(num_nodes, prob_connection):
    """
    Génère un graphe Erdos-Renyi.

    Args:
        num_nodes (int): Nombre total de sommets dans le graphe.
        prob_connection (float): Probabilité qu'une arête existe entre deux sommets.

    Returns:
        nx.Graph: Le graphe généré.
    """
    if prob_connection < 0 or prob_connection > 1:
        raise ValueError("La probabilité de connexion doit être entre 0 et 1.")
    return nx.erdos_renyi_graph(num_nodes, prob_connection, seed=42)
...
if __name__ == "__main__":
    ...
    prob_connection = 0.2  # Probabilité de connexion entre deux sommets
    graph_name_base = "erdos_renyi"  # Nom de base pour les fichiers DIMACS

    while True:
        try:
            # Génération du graphe Erdos-Rényi
            graph = generate_erdos_renyi_graph(num_nodes, prob_connection)
