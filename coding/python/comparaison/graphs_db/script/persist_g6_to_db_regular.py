...
def parse_regular_filename(filename):
    """Extrait vertex_cover, nb_sommets, degré et instance à partir du nom du fichier."""
    base = os.path.splitext(filename)[0]
    _, cover, n, d, instance = base.split("-")
    return int(cover), int(n), int(d), int(instance)
...
def insert_graph(db_name, file_path, root_dir):
    ...
    cover_size, nb_sommets, degree, instance_number = parse_regular_filename(graph_name_with_extension)
...
if __name__ == "__main__":
    root_directory = "g6_files/regular"
    db_name = "graphes.db"
